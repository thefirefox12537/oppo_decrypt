...
class ofpDecrypt(object):
    ...
    def main(self, filename, outdir):
        ...
        if self.ui:
            self.ui.finished.emit()
        else:
            print(f"Files successfully decrypted to subdirectory {outdir}")

    def manual(self, filename, outdir=None, isRead=False, target=None):
        if outdir == None:
...
class ofpGui(QWidget):
    ...
    def objectAdded(self, result, sender=None):
        if not sender or sender.objectName() == "ofp":
            ...
            self.textEdit.insertHtml(f"<p>Opened file : {self.ofp_LineEdit.text()}<br/></p>")
